...
public class RunControllerIntTest {
    ...
    void shouldFindRunById() {
        ...
        assertAll(
                ...
                () -> assertEquals("2026-02-20T06:05", run.startedOn().toString()),
                () -> assertEquals("2026-02-20T10:27", run.completedOn().toString()),
    ...
    void shouldUpdateExistingRun() {
        ...
        ResponseEntity<Void> updateRun = restClient.put()
                .uri("/api/runs/1")
                .body(run)
                .retrieve()
                .toBodilessEntity();

        assertEquals(204, updateRun.getStatusCodeValue());
    }
